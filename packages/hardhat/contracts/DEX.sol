...
contract DEX {
	...
	function init(uint256 tokens) public payable returns (uint256) {
		...
		require(tokens > 0, "DEX: must send tokens");
	...
	function ethToToken() public payable returns (uint256 tokenOutput) {
		...
		tokenOutput = price(msg.value, ethReserve, tokenReserves);
	...
	function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
		...
		ethOutput = price(tokenInput, tokenReserves, ethReserve);
	...
	function deposit() public payable returns (uint256 tokensDeposited) {
		...
		uint256 tokenReserves = token.balanceOf(address(this));
		uint256 ethReserve = address(this).balance - msg.value;
		uint256 tokenDeposit = ((msg.value * tokenReserves) / ethReserve) + 1;

		uint256 liquidityMinted = (msg.value / ethReserve) * totalLiquidity;
	...
	function withdraw(
		...
		uint256 ethWithdrawn = (amount * liquidity[msg.sender]) /
			totalLiquidity;
		uint256 tokenAmount = (ethWithdrawn * tokenReserve) / ethReserve;
